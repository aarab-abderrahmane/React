...
import { DELETE_STAGIAIRE } from "../features/stagiaireSlice";
import { useDispatch } from "react-redux";

const getScoreColor = (score) => {
  if (score >= 16)
    return "bg-emerald-300 text-emerald-900 border-2 border-emerald-900";
  if (score >= 12) return "bg-blue-300 text-blue-900 border-2 border-blue-900";
  if (score >= 10)
    return "bg-amber-300 text-amber-900 border-2 border-amber-900";
    ...
    <div className="bg-[var(--bg-secondary)] rounded-xl shadow-sm overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-[var(--bg-secondary)] border-b border-white/50">
              <th className="px-6 py-4 text-xs font-semibold text-white/70 uppercase tracking-wider">
                Stagiaire
              </th>
              <th className="px-6 py-4 text-xs font-semibold text-white/70 uppercase tracking-wider">
                Matricule
              ...
              <th className="px-6 py-4 text-xs font-semibold text-white/70 uppercase tracking-wider text-center">
                Score
              </th>
              <th className="px-6 py-4 text-xs font-semibold text-white/70 uppercase tracking-wider text-right">
                Actions
              </th>
            </tr>
          </thead>
        ...
        <div className="max-h-[500px] overflow-y-auto">
          <table className="w-full text-left border-collapse">
            <tbody className="divide-y divide-white/30 divide-dashed">
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-[#ff9b22] text-yellow-950 flex items-center justify-center font-bold text-xs">
                        ...
                        {stagiaire.nom[stagiaire.nom.length - 1].toUpperCase()}
                      </div>
                      <div>
                        <div className="font-medium text-[var(--text-color)] group-hover:text-black">
                          {stagiaire.nom}
                        </div>
                      </div>
                    </div>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-1.5 text-slate-300 group-hover:text-black">
                      ...
                      <span className="text-xs text-slate-400 group-hover:text-black/80">
                  ...
                  <td className="px-6 py-4 text-center">
                    <span
                      className={`inline-flex items-center justify-center px-4 py-1 rounded-full text-xs font-bold border ${getScoreColor(
                        stagiaire.moyenne
                      )}`}
                    >
                      {stagiaire.moyenne}
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          dispatch(DELETE_STAGIAIRE({ id: stagiaire.id }))
                        }
